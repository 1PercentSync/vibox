...
**Error 状态特殊处理**：
- 检查 `workspace.container_id` 字段是否存在
- **有 container_id**：脚本执行失败但容器还在运行 → 显示 [Terminal] 按钮（用于调试）+ [Reset] + [Delete]
- **无 container_id**：容器创建失败 → 只显示错误信息 + [Reset] + [Delete]
- 始终显示 `workspace.error` 字段内容作为错误描述
...
│  ├─────────────────┼──────┼───────────────────┤ │
│  │ VS Code Server  │ 8080 │ [Open] [Edit] [Del] │ │
│  │ Web App         │ 3000 │ [Open] [Edit] [Del] │ │
│  │ PostgreSQL      │ 5432 │ [Open] [Edit] [Del] │ │
│  └──────────────────────────────────────────────┘ │
│                                              │
│  Access Any Port                             │
│  ┌────────────────────────────────────────┐ │
...
- 点击 [Open] 在新窗口打开端口代理 URL（自动附带token）
- 点击 [Edit] 修改服务名称
- 点击 [Del] 删除该端口标签
- [+ Add] 添加新的端口标签
- "Access Any Port" 区域允许手动输入任意端口号访问
- 自动生成代理 URL：`/forward/{workspace-id}/{port}/?token={api-token}`
